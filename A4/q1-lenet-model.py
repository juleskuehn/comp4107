...
def model(X, w, w_2, w_3, w_4, w_fc, w_fc2, w_o, p_keep_conv, p_keep_hidden):
    ...
    l3a = tf.nn.relu(tf.nn.conv2d(l2, w_2, strides=[1, 1, 1, 1], padding='SAME'))
    print(l3a.shape)
    ...
    l5 = tf.reshape(l3, [-1, w_fc.get_shape().as_list()[0]])    # reshape to (?, 16x16x32)
...
w = init_weights([5, 5, 3, 6])   
w_2 = init_weights([5, 5, 6, 16])
# Don't use 3 and 4 for LeNet
w_3 = init_weights([3, 3, 64, 128])  
w_4 = init_weights([3, 3, 128, 128])   

w_fc = init_weights([16 * 8 * 8, 120])
w_fc2 = init_weights([120, 84])
w_o = init_weights([84, 10])         # 10 outputs (labels)

p_keep_conv = tf.placeholder("float")
p_keep_hidden = tf.placeholder("float")
py_x = model(X, w, w_2, w_3, w_4, w_fc, w_fc2, w_o, p_keep_conv, p_keep_hidden)
