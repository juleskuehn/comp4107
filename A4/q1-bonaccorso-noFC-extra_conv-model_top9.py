...
print(l4a.shape)
l4 = tf.nn.max_pool(l4a, ksize=[1, 2, 2, 1], strides=[
                    1, 2, 2, 1], padding='SAME')
print(l4.shape)
...
with tf.Session() as sess:
    ...
    # Most activated feature maps, layer 1
    # Code based on blog post: https://medium.com/@awjuliani/visualizing-neural-network-layer-activation-tensorflow-tutorial-d45f8bf7bbc4
    ...
    def getActivations(layer, stimuli):
        ...
        for i in range(units.shape[2]):
            reshapedUnits.append(units[:, :, i])

        largest = heapq.nlargest(9, reshapedUnits, key=lambda x: np.sum(x))
        plotNNFilter(largest)
    ...
    def plotNNFilter(units):
        ...
        plt.savefig(str(imageNum) + '_top_layers.png', dpi=100,
    ...
    for i in range(16):
        ...
        plt.tick_params(axis='both', left='off', top='off', right='off', bottom='off',
                        labelleft='off', labeltop='off', labelright='off', labelbottom='off')
        plt.imshow(teX[i], interpolation="nearest")
        fig = plt.gcf()
        fig.set_size_inches(2, 2)
        plt.savefig(str(imageNum) + '_input_image.png', dpi=100,
                    bbox_inches='tight', pad_inches=0.0)
        plt.show()
        getActivations(l1a, teX[i])
        imageNum += 1
