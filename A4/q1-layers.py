...
"""
Same layer configuration as the following Keras example:
...
def model(X, w, w_2, w_3, w_4, w_fc, w_o, p_keep_conv, p_keep_hidden):
    ...
    l5 = tf.reshape(l4, [-1, w_fc.get_shape().as_list()[0]])    # reshape to (?, 16x16x32)
...
w_o = init_weights([1024, 10])         # 10 outputs (labels)
