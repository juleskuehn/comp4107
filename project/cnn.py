...
def model(X, w, w_fc, w_o, p_keep_conv, p_keep_hidden):
    ...
    l3 = tf.reshape(l1, [-1, w_fc.get_shape().as_list()[0]])    
    print('l3', l3.shape)
...
trX = np.array(trX).reshape(-1, 96, 1, 1)
teX = np.array(teX).reshape(-1, 96, 1, 1)
trY = np.array(trY)
teY = np.array(teY)
...
w_fc = init_weights([32 * 48, 625])
w_o = init_weights([625, 32])      
