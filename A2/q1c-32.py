...
def trainTest(n_hidden=50, epochs=3000, lr=0.02, earlyStopping=False):
    ...
    with tf.Session() as sess:
        ...
        for step in range(epochs):
            ...
            if earlyStopping and step > 0 and validationCosts[-1] > validationCosts[-2]:
                consecFails += 1
...
def runExperiment(num_neurons, epochs, lr):
    ...
    for n in num_neurons:
        pred, mse, c, g = trainTest(n, epochs, lr)
        results.append((pred, mse, c, g))
        pred, mse, c, g = trainTest(n, epochs, lr, True)
        results.append((pred, mse, c, g))
    ...
    legends = [
        ...
        mpatches.Patch(color='red', label='without early stopping'),
        mpatches.Patch(color='green', label='with early stopping'),
    ]
