...
def digit_recognition(n_hidden = 15, training_noise = 3, lr = 0.01):
    ...
    with tf.Session() as sess:
        ...
        if VERBOSE:
            print("Finished step 1 with ", epoch1, " epochs")
        ...
        if VERBOSE:
            print("Finished step 2 with ", epoch2, " epochs")
        ...
        if VERBOSE:
            print("Finished step 3 with ", epoch3, " epochs")
...
def partC():
    ...
    plt.xlabel('Noise level')
    plt.ylabel('Percentage of recognition error')
    l1, = plt.plot(noises, t_acc, 'k--')
    l2, = plt.plot(noises, t_acc_noisy, 'r')
    plt.legend((l1, l2), ('trained without noise', 'trained with noise'))
    plt.show()
