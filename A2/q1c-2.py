...
def trainTest(n_hidden=50, epochs=3000, lr=0.02):
    ...
    with tf.Session() as sess:
        ...
        for step in range(epochs):
            ...
            # Early stopping: Check against test Data
            pred = sess.run([hy], feed_dict = {X: randomData[:,:2]})
            pred = [x[0] for x in pred[0]]
            mse = np.square(pred - randomData[:,2]).mean()
...
def runExperiment(num_neurons, epochs, lr):
    ...
    for n in num_neurons:
        pred, mse, costs = trainTest(n, epochs, lr)
        results.append((pred, mse, costs))
    ...
    plt.ylabel('MSE')
    plt.xlabel('Epochs')
    plt.plot(range(len(results[0][2])), results[0][2], 'k')
    plt.ylim(0,0.02)
    ...
    plt.ylabel('MSE')
    plt.xlabel('Epochs')
    plt.plot(range(len(results[1][2])), results[1][2], 'k')
    plt.ylim(0,0.02)
    ...
    plt.ylabel('MSE')
    plt.xlabel('Epochs')
    plt.plot(range(len(results[2][2])), results[2][2], 'k')
    plt.ylim(0,0.02)
    plt.show()


runExperiment([8, 50, 100], 100000, 0.02)